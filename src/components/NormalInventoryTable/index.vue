...
-->
<template>
  <div class="normal-inventory-table-component">
    <el-table
      ref="NormalInventoryTableComponent"
      :key="updateControl"
      :data="tableData"
      stripe
      border
    ...
    >
      ...
      <template v-for="item in tableHeader">
        ...
        <template v-else>
          ...
          >
            <template slot-scope="scope">
              <slot :name="item.prop" :row="scope.row" :column="scope.column" :index="scope.$index" :item="item">
    ...
    <div v-if="settingIconFlag" class="normal-inventory-table-setting" @click="handleShowTableSetting">
...
export default {
  name: 'NormalInventoryTableComponent',
  components: {
    TableColumnConfigurationDialog
  },
  props: {
  ...
  methods: {
     ...
     */
    addScrollListener() {
      this.$refs.NormalInventoryTableComponent?.bodyWrapper?.addEventListener('scroll', this.scrollFunction, false)
    },
    /**
     * 移除监听表格滚动
     * @param {void}
    ...
    removeScrollListener() {
      this.$refs.NormalInventoryTableComponent?.bodyWrapper?.removeEventListener('scroll', this.scrollFunction, false)
    },
    /**
    ...
    filterHeaderData(prop) {
      const result = this.tableHeader.filter((v) => v.prop === prop)
      return result
    },
    /**
     * 点击配置按钮
    ...
    handleMixinTableSettingData() {
      ...
      if (version === cacheVersion) {
        ...
        !isEmptyCacheData && _.merge(tableHeader, cacheData.data)
      } else {
        // 版本更新 清除缓存
...
.normal-inventory-table-component {
  width: 100%;
  height: 100%;
  position: relative;
}
.normal-inventory-table-setting {
  position: absolute;
  right: -10px;
  top: -10px;
  font-size: 20px;
...
.normal-inventory-table-component {
  ...
  // 输入框
  .el-table td div.el-input {
    input {
      width: 100%;
      height: 32px;
    }
  }
  .el-table td div.el-input-number {
    width: 100% !important;
  }
}
</style>
